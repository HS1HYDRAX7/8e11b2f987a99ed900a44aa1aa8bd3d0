...
local Window = WindUI:CreateWindow({
    ...
    Icon = "https://tr.rbxcdn.com/180DAY-f9b5451bff63b4224982521fc9801be8/150/150/Decal/Webp/noFilter",
    Author = "By Zenith Studios",
...
local ElysiumPlusTab = Window:Tab({ Title = "Elysium plus", Icon = "dollar-sign", Locked = false, })
...
local AbilitiesManagerTab = Window:Tab({ Title = "Abilities Manager", Icon = "blocks", Locked = false, })
...
MainTab:Paragraph({
    ...
    Desc = "! Links ¡",
    Thumbnail = "rbxassetid://81706308103426",
    ThumbnailSize = 85,
    Locked = false,
    Buttons = {
        {
            ...
            Title = "Website",
            Callback = function()
                setclipboard("https://hs1hydrax7.github.io/Zenith-Website/")
                WindUI:Notify({
                    Title = "Website",
                    Content = "Link Copied",
                    Duration = 3,
        ...
        },
        {
            ...
            Title = "Discord",
            Callback = function()
                setclipboard("https://discord.gg/D97k9TTCq3")
                WindUI:Notify({
                    Title = "Discord",
...
local InterfaceSec = MainTab:Section({
    Title = "Interface customization",
    Icon = "paintbrush",
    Opened = true,
})

InterfaceSec:Dropdown({
    Title = "Theme",
...
InterfaceSec:Toggle({
    ...
    Value = true,
    Callback = function(h)
        Window:HideSearchBar(h)
    end
})

local UserSection = MainTab:Section({
    Title = "User Profile",
    Icon = "shield-user",
    Opened = true,
})
...
local gamePassId = 1696760402
local LocalPlayer = Players.LocalPlayer
local elysiumplus = "https://www.roblox.com/game-pass/1696760402/elysium-plus"
...
local function CopyLink()
    pcall(function()
        setclipboard(elysiumplus)
    end)
end

local elysiumplusUnlocked = hasGamepass()

local ElysiumSec = ElysiumPlusTab:Section({
    Title = "Elysium Plus",
    Icon = "dollar-sign",
})

ElysiumSec:Paragraph({
    Title = "Elysium Plus",
    Desc = "Currently, you can only purchase elysium plus through gamepass",
    Image = "rbxassetid://101473576944404",
    ImageSize = 80,
    Locked = false,
})
...
ElysiumSec:Button({
    Title = "Buy elysium plus",
    Desc = "Unlock exclusive features",
    Callback = function()
        if elysiumplusUnlocked then
            WindUI:Notify({
                Title = "Elysium plus",
                Content = "You already own elysium plus",
                Duration = 3,
                Icon = "rbxassetid://125746152594286",
            })
            return
        end
        ...
        WindUI:Notify({
            Title = "Elysium plus",
            Content = "Link copied! Paste into your browser to purchase.",
            Duration = 5,
            Icon = "rbxassetid://125746152594286",
        })
...
local function elysiumplusFeature(callback)
    if not elysiumplusUnlocked then
        WindUI:Notify({
            Title = "Elysium plus",
            Content = "Unlock elysium plus first",
            Duration = 3,
            Icon = "rbxassetid://125746152594286",
        })
        return
...
local BlockSec = ElysiumSec:Section({
    Title = "Auto block",
})
        ...
        elysiumplusFeature(function()
            ...
            if state == true then
                ...
                local function TryBlock(killerHRP)
                    ...
                    local mag = (killerHRP.Position - hrp.Position).Magnitude
                    if mag >= tonumber(_G.AutoBlockRange) then return end

                    local unit = ((hrp.Position - killerHRP.Position) * Vector3.new(1,0,1)).Unit
                    local lookvector = killerHRP.CFrame.LookVector * Vector3.new(1,0,1)
                    if lookvector:Dot(unit) <= 0.75 then return end
                ...
                local function HookOnto(chr)
                    local timeout = 0
                    while not chr:FindFirstChild("HumanoidRootPart") and timeout < 5 do
                        task.wait(0.1)
                        timeout += 0.1
                    end
                    ...
                    local chrn = chr.Name
                    if not workspace.GameAssets.Teams.Killer:FindFirstChild(chrn) then return end

                    local connections = {}
                    _G.AutoBlockConnections[chrn] = connections

                    local swing = hrp:FindFirstChild("Swing")
                    if not swing then
                        local t2 = 0
                        while not hrp:FindFirstChild("Swing") and t2 < 3 do
                            task.wait(0.1)
                            t2 += 0.1
                        end
                        swing = hrp:FindFirstChild("Swing")
                    end

                    if swing and swing:IsA("Sound") then
                        local c1 = swing.Played:Connect(function()
                            TryBlock(hrp)
                        end)
                        ...
                        local loopConn = game:GetService("RunService").Heartbeat:Connect(function()
                            if not _G.AutoBlockEnabled or _G.AutoBlockUnhook then return end
                            local isPlaying = swing.IsPlaying
                    ...
                    repeat task.wait(0.2) until
                        not workspace.GameAssets.Teams.Killer:FindFirstChild(chrn)
                        or _G.AutoBlockUnhook == true

                    for _, conn in next, connections do
                        pcall(function() conn:Disconnect() end)
                    end
                    _G.AutoBlockConnections[chrn] = nil
                end

                for _, v in next, workspace.GameAssets.Teams.Killer:GetChildren() do
                    task.spawn(HookOnto, v)
                end

                local killerAddedConn = workspace.GameAssets.Teams.Killer.ChildAdded:Connect(function(v)
                    if _G.AutoBlockEnabled and not _G.AutoBlockUnhook then
                        task.spawn(HookOnto, v)
                    end
                end)
                _G.AutoBlockKillerAddedConn = killerAddedConn

            else
                _G.AutoBlockUnhook = true

                if _G.AutoBlockKillerAddedConn then
                    _G.AutoBlockKillerAddedConn:Disconnect()
                    _G.AutoBlockKillerAddedConn = nil
                end

                if _G.AutoBlockConnections then
                    for _, conns in next, _G.AutoBlockConnections do
    ...
    Callback = function(state)
        ...
        if state then
            for _,v in ipairs(killers:GetChildren()) do
                if v:IsA("Model") and not v:FindFirstChild("ESP_HIGHLIGHT") then
...
VisualsSection:Toggle({
    ...
    Callback = function(state)
        ...
        local function protect(char)
            ...
            if _G.ignoreAddedConn then
                _G.ignoreAddedConn:Disconnect()
            end

            _G.ignoreAddedConn = char.ChildAdded:Connect(function(child)
                if not _G.ignoreSelf then return end
        ...
        if state then
            ...
            if _G.ignoreCharConn then
                _G.ignoreCharConn:Disconnect()
            end

            _G.ignoreCharConn = player.CharacterAdded:Connect(function(char)
                task.wait(0.1)
                protect(char)
            end)
            ...
            if _G.ignoreCharConn then
                _G.ignoreCharConn:Disconnect()
                _G.ignoreCharConn = nil
            end

            if _G.ignoreAddedConn then
                _G.ignoreAddedConn:Disconnect()
                _G.ignoreAddedConn = nil
            end
...
local AbilitiesSec = AbilitiesManagerTab:Section({
    Title = "Abilities Manager",
    Icon = "blocks",
    Opened = true,
})

AbilitiesSec:Dropdown({
...
local oneonecard = nil
local onetwocard = nil

local twoonecard = nil
local twotwocard = nil

local threeonecard = nil
local threetwocard = nil

local CardSec = AbilitiesManagerTab:Section({
    Title = "Summon Cards",
    Desc = "Summon all Selected when the Round Starts \n(PRESS THE BUTTON TO SUMMON)",
    Icon = "id-card-lanyard",
    Opened = false
})
 
local OneCardDeck = CardSec:Section({
     Title = "1° Deck",
     Icon = "diamond"
})

local TwoCardDeck = CardSec:Section({
     Title = "2° Deck",
     Icon = "club"
})

local ThreeCardDeck = CardSec:Section({
     Title = "3° Deck",
     Icon = "spade"
})
...
local presets = {
    ...
    Ghost = {
        ...
        Walk = "rbxassetid://124260679864309",
        Idle = "rbxassetid://110395159339100"
    }
}

local AnimSec = AnimationsTab:Section({
    Title = "Animations",
    Icon = "play",
    Opened = true,
})
...
local MusicFolder = "Elysium X Rework/assets/musics"
...
local MusicSec = MiscTab:Section({
    Title = "Music player",
    Icon = "music",
})

local MusicInfo = MusicSec:Paragraph({
    Title = "Now playing",
    Desc = "00:00/00:00"
})
...
local MusicDropdown = MusicSec:Dropdown({
    Title = "Musics",
...
local HStack1 = MusicSec:HStack()

local VStack1 = HStack1:VStack()
local VStack2 = HStack1:VStack()
...
local AvatarSec = MiscTab:Section({
    Title = "Avatar Copier",
    Icon = "copy",
    Opened = true,
})
...
AvatarSec:Button({
    ...
    Callback = function()
        ...
        if not _G._avatarUser or _G._avatarUser == "" then return end
        ...
        local function apply_avatar(username)
            task.spawn(function()
                ...
                if not _G._originalDesc then
                    _G._originalDesc = hum:GetAppliedDescription()
                end
        ...
        _G._lastAvatarUser = _G._avatarUser
        apply_avatar(_G._avatarUser)

        if _G._avatarRespawnConn then
            _G._avatarRespawnConn:Disconnect()
        end

        _G._avatarRespawnConn = Players.LocalPlayer.CharacterAdded:Connect(function()
            task.wait(0.65)
            if _G._lastAvatarUser then
                apply_avatar(_G._lastAvatarUser)
            end
        end)
...
AvatarSec:Button({
    ...
    Callback = function()
        ...
        if not _G._originalDesc then return end
        ...
        pcall(function()
            if hum.ApplyDescriptionClientServer then
                hum:ApplyDescriptionClientServer(_G._originalDesc)
            else
                hum:ApplyDescription(_G._originalDesc)
            end
        end)

        if _G._avatarRespawnConn then
            _G._avatarRespawnConn:Disconnect()
            _G._avatarRespawnConn = nil
        end

    end
})

local SettingSec = SettingsTab:Section({
...
local MinorSec = SettingsTab:Section({
